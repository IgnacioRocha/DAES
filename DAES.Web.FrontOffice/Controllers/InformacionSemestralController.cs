...
{
    [Audit]
    public class InformacionSemestralController : Controller
    {
        ...
        {
            Global.CurrentClaveUnica.ClaveUnicaRequestAutorization.controller = "InformacionSemestral";
        ...
        public ActionResult Create(int id)
        {
            ...
            ViewBag.Periodo = new SelectList(_db.Periodo.Where(q=>q.Tipo == "InformacionSemestral").OrderByDescending(q => q.PeriodoId), "Descripcion", "Descripcion");
            ViewBag.RegionSolicitanteId = new SelectList(_db.Region.OrderBy(q => q.Nombre), "RegionId", "Nombre");

            return View(new Model.DTO.DTOInformacionSemestral()
            {
        ...
        public ActionResult Create(Model.DTO.DTOInformacionSemestral model)
        {
            if (!Global.CurrentClaveUnica.IsAutenticated)
            {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    DefinicionProcesoId = (int)Infrastructure.Enum.DefinicionProceso.InformacionACSemestral,
                    OrganizacionId = model.OrganizacionId,
                ...
                SetFile(model.EstadosFinancierosIntermedios, (int)Infrastructure.Enum.TipoDocumento.EstadosFinancierosIntermedios);
                SetFile(model.FichaDetalladaFuentesFinanciamiento, (int)Infrastructure.Enum.TipoDocumento.FichaDetalladaFuentesFinanciamiento);
