...
{
    [Audit]
    public class DisolucionCooperativaController : Controller
    {
        ...
        public ActionResult Create(Model.DTO.DTODisolucionCooperativa model)
        {
            if (!Global.CurrentClaveUnica.IsAutenticated)
            {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    DefinicionProcesoId = (int)Infrastructure.Enum.DefinicionProceso.DisolucionCooperativa,
                    OrganizacionId = model.OrganizacionId
                };
